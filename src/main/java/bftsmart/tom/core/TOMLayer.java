...
public final class TOMLayer extends Thread implements RequestReceiver {
    ...
    // PriorityQueue (MaxHeap) to store all running instances.
    private PriorityQueue<Integer> executingInstances = new PriorityQueue<>(Comparator.reverseOrder());
    ...
    public void setInExec(int inEx) {
        proposeLock.lock();
        this.executingInstances.add(inEx);
        if (pipelineManager.isAllowedtoStartNewConsensus() && !isRetrievingState()) {
            this.pipelineManager.addToConsensusInExec(inEx);
    ...
    public void removeInExec(int inExToRemove) {
        proposeLock.lock();
        if (inExToRemove != -1)
            this.pipelineManager.removeFromConsensusInExecList(inExToRemove);
        this.executingInstances.remove(inExToRemove);
        if (pipelineManager.isAllowedtoStartNewConsensus() && !isRetrievingState()) {
            canPropose.signalAll();
        }
        proposeLock.unlock();
    ...
    public int getInExec() {
        ...
        if (this.executingInstances.size() == 0) {
            return -1; // indicate that no consensus instance is in execution
        } else {
            return this.executingInstances.peek(); // return highest ongoing instance?
        }
    }
...
//                int execId = getLastExec() + 1;
               // int execId = getLastExec() + (pipelineManager.getConsensusesInExecution().isEmpty() ? 1 : (pipelineManager.getConsensusesInExecution().size() + 1));
                int execId = this.executingInstances.peek() != null
                        ?  Math.max(this.executingInstances.peek()  + 1, getLastExec() + 1)
                        : getLastExec() + 1;
                setInExec(execId);
...
//                CHECK IT AGAIN.: looks like working good.
    ...
    public PriorityQueue<Integer> getAllExecutingInstances() {
        return this.executingInstances;
    }
